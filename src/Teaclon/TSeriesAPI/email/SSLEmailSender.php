...
* $mail->setMail("主题", "内容", "发送原因", (bool = false) 是否使用邮件模板);     // 设置邮件主题、内容
* $mail->sendMail();                         // 发送
*/


namespace Teaclon\Supervisor\utils\email;

use Teaclon\Supervisor\Main;

class SSLEmailSender
{
	
	protected $_userName;		 // 邮件传输代理用户名
	protected $_password;		 // 邮件传输代理密码
	protected $_sendServer;	     // 邮件传输代理服务器地址
	protected $_port;			 // 邮件传输代理服务器端口
	protected $_from;			 // 发件人
	protected $sender;		     // 发件者称呼
	protected $_to = [];		 // 收件人
	protected $_cc = [];		 // 抄送
	protected $_bcc = [];		 // 秘密抄送
	...
	protected $_isSecurity;	     // 是否是安全连接
	protected $_errorMessage;	 // 错误信息
	
	private static $display_logger = \null;   // 是否显示错误信息;
	
	private $plugin, $logger, $prefix;
	
	public function __construct(Main $plugin, $debubg_mode)
	{
		$this->plugin = $plugin;
		$this->prefix = $plugin::NORMAL_PRE;
		$this->logger = $plugin->getServer()->getLogger();
		self::$display_logger = $debubg_mode;
	}
	...
	**/
	...
	/**
	  ...
	  * @param string $reason 用户收到邮件的原因
	  * @param string $subject 邮件主体内容, 可以是纯文本, 也可是是HTML文本
	  * @return boolean
	**/
	public function setMail($subject, $body, $reason = "欢迎邮件", bool $format = false)
	{
		if($format) $this->sendMessageToServer("Format - Demo is open.");
		$this->_subject = base64_encode($subject);
		$this->_body = (!$format) ? base64_encode($body) : base64_encode($this->plugin->dgapi->getDemo($body, $reason, $this->sender));
		return true;
	}
	...
	**/
	...
	{
		$this->sendMessageToServer("Send command: {$command}, expected code: {$code}");
		//发送命令给服务器
		try
		{
			if(socket_write($this->_socket, $command, strlen($command)))
			{
				...
				$this->sendMessageToServer("response: {$data}");
				if($data)
				{
					$pattern = "/^".$code."+?/";
	...
	**/
	...
	public function sendMessageToServer(string $msg, string $level = "info", bool $logger = false)
	{
		if(self::$display_logger)
		{
			$msg = str_replace("\n", "", $msg);
			$msg = str_replace("\r", "", $msg);
			$this->plugin->ssm($this->prefix."[EMailSender] ".$msg, $level, "server");
		}
	}
}
?>